...
const fetchContagens = createServerFn().handler(async () => {
  ...
  try {
    const [dataResponse, pcrCounts, atlasData] = await Promise.all([
      cmsFetch<any>(COUNTINGS_PAGE_DATA, {
        ttl: 300,
        timeout: 5000,
        onError: tracker.at(COUNTINGS_PAGE_DATA),
      }),
      (async () => {
        try {
          const response = await fetch(PCR_CONTAGENS_URL);
      ...
      cmsFetch<any>(COUNTINGS_ATLAS_LOCATIONS, {
        ...
        timeout: 5000,
        fallback: [],
        onError: tracker.at(COUNTINGS_ATLAS_LOCATIONS),
      }),
    ]);
