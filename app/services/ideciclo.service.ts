...
export function getRatesSummary(structure: any, forms: any) {
  const form = forms;
  const rate = structure.reviews[structure.reviews.length - 1].rates;
  const last_rate = structure.reviews.length > 1 
    ? structure.reviews[structure.reviews.length - 2].rates 
    : rate;
  ...
  const series: any[] = [];
  const reviews = structure.reviews.slice().reverse();
  ...
  reviews.forEach((r: any, index: any) => {
    ...
    series.push({ type, name: r.year, data: d });
  });

  const date: any = new Date(form.header.date);

  return {
    tipologia: structure.tipologia,
...
export function structureStatistics(structure: any, info: any) {
