...
export const contagemCompareQueryOptions = (slug: string, compareSlug: string) =>
  queryOptions({
    ...
    queryFn: async () => {
      ...
      const [data, pageData, boxes] = await Promise.all([
        Promise.all(
        ...
        fetchData(),
        compareContagensLoader({ params: { slug, compareSlug } }),
      ]);
...
export const loader = async ({ params }: { params: { slug: string; compareSlug: string } }) =>
  contagemCompareQueryOptions(params.slug, params.compareSlug).queryFn({} as any);
