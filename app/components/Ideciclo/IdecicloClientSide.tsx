...
export default function IdecicloClientSide({ cidades, structures, ideciclo }: any) {
    ...
    useEffect(() => {
        if (selectedCity) {
            let city_structures = structures.filter((s: any) => {
    ...
    const cards_city = (chosenCities: any) => {
        const orderedCities = chosenCities
            .filter((f: any) => f.reviews[0].ideciclo > 0)
            .sort((a: any, b: any) => {
    ...
    const statesFilter = {
        ...
        value: cityState,
        name: "cityState",
        onChange: (e: any) => setCityState(e.target.value),
        onBlur: (e: any) => e,
    ...
    const CitiesRanking = {
        ...
        filters: [
            ...
            },
            {
                ...
                value: cityPop,
                name: "city_pop",
                onChange: (e: any) => setCityPop(e.target.value),
                onBlur: (e: any) => e,
                items: [
                    { value: "", label: "Todas" },
    ...
    function SelectionFilterIdeciclo({ title, value, name, onChange, items }: any) {
        return (
            <div className="relative rounded-lg mx-4 m-4 xl:m-8 max-w-md">
                ...
                <div className="relative z-10 text-white font-bold rounded-sm px-4 pb-6 pt-2 mx-4">
                    <label htmlFor={value}>{title}</label>
                    <select
                        className="block appearance-none text-black font-bold w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded-sm shadow-sm leading-tight focus:outline-hidden focus:shadow-outline"
                        value={value}
                        name={name}
                        onChange={onChange}
                    >
    ...
    function MultipleSelectionFiltersIdeciclo({ title = "", filters }: any) {
        return (
            <section className="flex gap-2 relative justify-center mb-10 mx-auto w-full 2xl:w-3/4 ">
                ...
                <div className="mx-auto flex flex-col justify-center align-middle gap-2 md:gap-5 relative z-2">
                    ...
                    <div className="flex flex-wrap align-baseline gap-0 md-gap-10 justify-center grow mx-auto">
                        {filters.length > 0 &&
    ...
    const NumberCardIdeciclo = ({
        ...
        return (
            <div
                className={`flex flex-col rounded-[40px] shadow-[0px_6px_8px_rgba(0,0,0,0.25)] h-full mx-3 ld:mx-0 p-3 justify-center align-center ${selected ? "bg-[#6DBFAC] text-gray-700" : "bg-white text-gray-700"
    ...
    const NumberCardsIdeciclo = ({ cards, data, selected, options }: any) => {
        return (
    ...
    function cityStatistics(selectedCity: any) {
